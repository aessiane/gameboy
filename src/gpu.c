...
{
  unsigned const fb = UPPER_BOUND(PIXEL_FIFO_SIZE - gb->gpu.pff.tail,
                                  FETCHED_PIXEL_SIZE);
...
static inline uint8_t get_pixel_color(t_gameboy *gb, uint8_t palette,
                                      uint8_t value)
{
...
/*
 ** If this slows down the emulator at some point,, do it step by step, by
 ** breaking down the OAM searching to one call per cycle
 */
...
sprite_loop:
      if (sprite != gb->gpu.oam.index) {
          t_oam_entry const *entry =
            (t_oam_entry const *)(gb->memory.start + gb->gpu.oam.array[sprite]);
  ...
  switch (mode) {
    ...
    case GM_HBLANK:
        if (gb->gpu.mode_counter >= 204) {
            ...
            if (*gb->hregisters.ly == SCREEN_HEIGHT - 1) {
                *gb->hregisters.stat =
                  SET_STAT_MODE(*gb->hregisters.stat, GM_VBLANK);
                refresh_screen(gb);
                clear_screen(gb);
                joypad_step(gb);
                timing(gb);
                ...
                *gb->hregisters.stat =
                  SET_STAT_MODE(*gb->hregisters.stat, GM_OAM_SEARCH);
            }
        }
        break;
        // V-Blank
    ...
    case GM_OAM_SEARCH:
        if (gb->gpu.mode_counter >= 80) {
            ...
            *gb->hregisters.stat =
              SET_STAT_MODE(*gb->hregisters.stat, GM_PIXEL_TRANSFER);
        }
        break;
        // Pixel transfer
    case GM_PIXEL_TRANSFER:
        if (gb->gpu.mode_counter >= 172) {
            ...
            draw_line(gb);
            *gb->hregisters.stat =
              SET_STAT_MODE(*gb->hregisters.stat, GM_HBLANK);
        }
        break;
  }
}
...
{
  uint8_t lcdc = *gb->hregisters.lcdc;
...
bool start_gpu(t_gameboy *gb)
{
  ...
  gb->gpu.display.screen = SDL_SetVideoMode(gb->gpu.display.width,
                                            gb->gpu.display.height,
                                            8,
